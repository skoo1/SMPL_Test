...
// ---------------------------------------------
// 내부 함수 선언부 (필요하면 헤더로 뺄 수 있음)
static void shapeblend(
...
SMPLModel buildSMPLModel()
{
    nlohmann::json data;
    {
        ...
        if (!f.is_open()) {
            std::cerr << "[buildSMPLModel] Cannot open file: model.json" << std::endl;
            // 여기서 return SMPLModel() 하거나 throw 해도 됨
            // 간단히 빈 모델 리턴할 수도 있음
        }
        else {
            f >> data;
...
// ----------------------------------------------------------------------------
// 아래는 내부 static 함수들 (shape/pose blend, transforms 등)
// ----------------------------------------------------------------------------
...
// poseblend: axis-angle -> quaternion -> posedirs
static Eigen::Vector4d axis2quat_single(const Eigen::Vector3d& axisAngle)
{
