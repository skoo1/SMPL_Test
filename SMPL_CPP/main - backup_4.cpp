...
SMPLModel buildSMPLModel(const nlohmann::json& data)
{
    ...
    {
        std::cout << "posedirs row " << row << ": ";
        for(int d = 0; d < posedim; d++)
        {
            std::cout << posedirs(row, d) << " ";
        }
        std::cout << std::endl;
    }
...
/**
 * @brief Eigen MatrixXd -> std::vector<double> (2D를 [rows,cols]로 flatten)
 *        (row-major 순서)
 */
std::vector<double> eigenMatrixToStdVector(const Eigen::MatrixXd& mat)
{
    std::vector<double> result;
    result.reserve(mat.rows() * mat.cols());
...
/**
 * @brief SMPL에서 사용하는 (24 x 16) 매트릭스를
 *        MATLAB처럼 (24 x 4 x 4) 로 해석해 저장하기 위한 헬퍼 함수
 *        - shape = [24,4,4]
 *        - data는 (i, r, c) 순으로 flatten (row-major 유사 개념)
 *
 *   Note: MATLAB은 column-major 이지만,
 *         여기서는 "i -> r -> c 순으로" 나열해서
 *         단순히 3차원 재구성할 수 있도록 했습니다.
 *         필요하다면 실제 MATLAB에서 reshape 시 주의하세요.
 */
std::vector<double> eigenMatrixToStdVector_3d_24x4x4(const Eigen::MatrixXd& mat)
{
    // mat.rows() == 24, mat.cols() == 16 인 상태를 가정
    // shape = [24, 4, 4]
    std::vector<double> result;
    result.reserve(24 * 4 * 4);
    for (int i = 0; i < 24; ++i)
    {
        // i번째 "행" 이라고 생각하고,
        // 16개 컬럼을 4x4로 쪼갬
        //   global_transform(i, 0..15)
        //   => row=r, col=c 로 변환 시 index = r*4 + c
        for (int r = 0; r < 4; ++r)
        {
            for (int c = 0; c < 4; ++c)
            {
                result.push_back(mat(i, r * 4 + c));
            }
        }
    }
    return result;
...
)
{
    ...
    // --- 기존 콘솔 출력 (원하시는 로그) ---
    std::cout << "[SMPL_Calc] thetas.size(): " << thetas.size() << std::endl;
    std::cout << "[SMPL_Calc] betas.size(): "  << betas.size()  << std::endl;
    std::cout << "[SMPL_Calc] model.K: "       << model.K       << std::endl;
    std::cout << "[SMPL_Calc] j_shaped.size(): " << j_shaped.size() << std::endl;
    std::cout << "[SMPL_Calc] v_shaped.size(): " << v_shaped.size() << std::endl;
    std::cout << "[SMPL_Calc] global_transform  : (" 
              << global_transform.rows() << " x " 
              << global_transform.cols() << ")" << std::endl;
    std::cout << "[SMPL_Calc] global_transform_remove: (" 
              << global_transform_remove.rows() << " x " 
              << global_transform_remove.cols() << ")" << std::endl;
    std::cout << "[SMPL_Calc] v_shaped2.size(): " << v_shaped2.size() << std::endl;
    std::cout << "[SMPL_Calc] v_rot: (" 
              << v_rot.rows() << " x " 
              << v_rot.cols() << ")" << std::endl;
    std::cout << "[SMPL_Calc] verts: (" 
              << verts.rows() << " x " 
              << verts.cols() << ")" << std::endl;
    std::cout << "[SMPL_Calc] joints.size(): " 
              << joints.size() << " (expected " 
              << model.K * 3 << ")" << std::endl;

    // ---  (2) MATLAB과 비슷한 형태로 JSON 저장 ---
    nlohmann::json j_matlab;

    // thetas (72x1)
    {
        j_matlab["thetas"]["shape"] = { static_cast<int>(thetas.size()), 1 };
        j_matlab["thetas"]["data"]  = eigenVectorToStdVector(thetas);
    }
    // betas (300x1)
    {
        j_matlab["betas"]["shape"] = { static_cast<int>(betas.size()), 1 };
        j_matlab["betas"]["data"]  = eigenVectorToStdVector(betas);
    }
    // j_shaped (96x1)
    {
        j_matlab["j_shaped"]["shape"] = { static_cast<int>(j_shaped.size()), 1 };
        j_matlab["j_shaped"]["data"]  = eigenVectorToStdVector(j_shaped);
    }
    // v_shaped (27560x1)
    {
        j_matlab["v_shaped"]["shape"] = { static_cast<int>(v_shaped.size()), 1 };
        j_matlab["v_shaped"]["data"]  = eigenVectorToStdVector(v_shaped);
    }
    // v_shaped2 (27560x1)
    {
        j_matlab["v_shaped2"]["shape"] = { static_cast<int>(v_shaped2.size()), 1 };
        j_matlab["v_shaped2"]["data"]  = eigenVectorToStdVector(v_shaped2);
    }
    // v_rot (6890x4)
    {
        j_matlab["v_rot"]["shape"] = { v_rot.rows(), v_rot.cols() };
        j_matlab["v_rot"]["data"]  = eigenMatrixToStdVector(v_rot);
    }
    // verts (6890x4)
    {
        j_matlab["verts"]["shape"] = { verts.rows(), verts.cols() };
        j_matlab["verts"]["data"]  = eigenMatrixToStdVector(verts);
    }
    // joints (72x1)
    {
        j_matlab["joints"]["shape"] = { static_cast<int>(joints.size()), 1 };
        j_matlab["joints"]["data"]  = eigenVectorToStdVector(joints);
    }

    // global_transform (24x4x4 in MATLAB)
    {
        // 실제 C++에서 (24 x 16) 2D 행렬이지만,
        // MATLAB에서는 (24 x 4 x 4)로 사용한다고 했으니 3D로 저장.
        j_matlab["global_transform"]["shape"] = { 24, 4, 4 };
        j_matlab["global_transform"]["data"]  = eigenMatrixToStdVector_3d_24x4x4(global_transform);
    }
    // global_transform_remove (24x4x4 in MATLAB)
    {
        j_matlab["global_transform_remove"]["shape"] = { 24, 4, 4 };
        j_matlab["global_transform_remove"]["data"]  = eigenMatrixToStdVector_3d_24x4x4(global_transform_remove);
    }

    // --- JSON 결과를 콘솔에 출력해볼 수도 있음 (주의: 용량 큼) ---
    // std::cout << j_matlab.dump(2) << std::endl;

    // --- 파일로 저장하고 싶다면 (예: smpl_calc_matlab.json) ---
    {
        std::ofstream ofs("smpl_calc_matlab.json");
        if(ofs.is_open())
        {
            ofs << j_matlab.dump(2); 
            ofs.close();
            std::cout << "[SMPL_Calc] JSON saved to smpl_calc_matlab.json" << std::endl;
        }
        else
        {
            std::cerr << "[SMPL_Calc] Failed to open file for writing JSON" << std::endl;
        }
    }
}
...
{
    // [1] pose = thetas 의 일부 segment
    Eigen::VectorXd pose = thetas.segment(3, thetas.size()-3); // 필요하다면 조정

    // [2] nJoints 계산
    int nJoints = static_cast<int>(pose.size() / 3);

    // [3] quaternionAngle (4 * nJoints)
    ...
    // [4] shape_feat 예시
    double shape_feat = betas(0); // 또는 betas(1) 등, 필요에 따라 변경

    // [5] feat = [ quaternionAngle; shape_feat ]
    Eigen::VectorXd feat(4*nJoints + 1);
    feat.head(4*nJoints) = quaternionAngle;
    feat(4*nJoints)      = shape_feat;

    // [6] 최종 결과: v_shaped2 = v_shaped + model.posedirs_vec * feat
    v_shaped2 = v_shaped + model.posedirs_vec * feat;

    // ----------------------------------------------------------------------------
    // 추가: JSON에 poseblend 관련 변수들 저장
    // ----------------------------------------------------------------------------
    nlohmann::json j_poseblend;

    // pose
    j_poseblend["pose"]["size"] = static_cast<int>(pose.size());
    j_poseblend["pose"]["data"] = eigenVectorToStdVector(pose);

    // nJoints
    j_poseblend["nJoints"] = nJoints;

    // quaternionAngle
    j_poseblend["quaternionAngle"]["size"] = static_cast<int>(quaternionAngle.size());
    j_poseblend["quaternionAngle"]["data"] = eigenVectorToStdVector(quaternionAngle);

    // shape_feat
    j_poseblend["shape_feat"] = shape_feat;

    // feat
    j_poseblend["feat"]["size"] = static_cast<int>(feat.size());
    j_poseblend["feat"]["data"] = eigenVectorToStdVector(feat);

    // v_shaped
    j_poseblend["v_shaped"]["size"] = static_cast<int>(v_shaped.size());
    j_poseblend["v_shaped"]["data"] = eigenVectorToStdVector(v_shaped);

    // posedirs_vec (추가!)
    j_poseblend["posedirs_vec"]["rows"] = model.posedirs_vec.rows();
    j_poseblend["posedirs_vec"]["cols"] = model.posedirs_vec.cols();
    j_poseblend["posedirs_vec"]["data"] = eigenMatrixToStdVector(model.posedirs_vec);

    // v_shaped2
    j_poseblend["v_shaped2"]["size"] = static_cast<int>(v_shaped2.size());
    j_poseblend["v_shaped2"]["data"] = eigenVectorToStdVector(v_shaped2);

    // ----------------------------------------------------------------------------
    // 파일 저장
    // ----------------------------------------------------------------------------
    std::ofstream ofs("poseblend.json");
    if (ofs.is_open()) {
        ofs << j_poseblend.dump(2);
        ofs.close();
        std::cout << "[poseblend] JSON saved to poseblend.json" << std::endl;
    } else {
        std::cerr << "[poseblend] Failed to open file for writing JSON" << std::endl;
    }
}
