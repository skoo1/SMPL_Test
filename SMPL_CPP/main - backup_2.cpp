...
{
    {
        ...
        std::cout << "    ";
        // MATLAB에서 thetas(1:10), C++은 인덱스 0부터 시작하므로 0~9
        // 크기가 10보다 작다면 가능한 범위만 출력
        for (int i = 0; i < std::min<Eigen::Index>(10, thetas.size()); i++)
        {
            std::cout << thetas(i) << " ";
        }
        std::cout << std::endl;
        ...
        std::cout << "    ";
        for (int i = 0; i < std::min<Eigen::Index>(10, betas.size()); i++)
        {
            std::cout << betas(i) << " ";
        }
        std::cout << std::endl;
    ...
    {
        std::cout << "j_shaped (first 10) = ";
        // MATLAB 코드: disp(j_shaped(1:10,1)')
        // C++에서는 (j_shaped.size() >= 10)라 가정
        for(int i=0; i<10 && i<j_shaped.size(); i++){
            std::cout << j_shaped[i] << " ";
        }
        std::cout << std::endl;

        std::cout << "v_shaped (first 10) = ";
        // MATLAB 코드: disp(v_shaped(1:10, 1)')
        for(int i=0; i<10 && i<v_shaped.size(); i++){
            std::cout << v_shaped[i] << " ";
        }
        std::cout << std::endl;
    }
    ...
    /*
    {
        std::cout << "global_transform first row: ";
        for(int c = 0; c < 16; c++)
        {
            // (row=1, col=c) 접근
            double val = global_transform(0, c);
            std::cout << val << " ";
        }
        std::cout << std::endl;

        std::cout << "global_transform_remove first row: ";
        for(int c = 0; c < 16; c++)
        {
            // (row=1, col=c) 접근
            double val = global_transform_remove(0, c);
            std::cout << val << " ";
        }
        std::cout << std::endl;
    }
    */
    ...
    {
        std::cout << "v_shaped2 (first 10) = ";
        // MATLAB 코드: disp(v_shaped2(1:10, 1)')
        for(int i=0; i<10 && i<v_shaped2.size(); i++){
            std::cout << v_shaped2[i] << " ";
        }
        std::cout << std::endl;
    }
    ...
    // --- Debug print: v_rot(0..9, 0) ---
    std::cout << "v_rot (first 10 rows, col=0):\n";
    int maxRowsToShow = std::min<int>(10, v_rot.rows());
    for(int i = 0; i < maxRowsToShow; i++)
    {
        double val = v_rot(i, 0); // (i번째 행, 0번째 열)
        std::cout << val << "\n";
    }
    std::cout << std::endl;
